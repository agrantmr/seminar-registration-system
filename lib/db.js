...
async function addRegistration(firstName, email, ipAddress) {
  try {
    ...
    const result = await sql.transaction([
      sql`BEGIN`,
      sql`SELECT COUNT(*) as count FROM registrations FOR UPDATE`,
      sql`INSERT INTO registrations (first_name, email, ip_address)
          VALUES (${firstName}, ${email.toLowerCase()}, ${ipAddress})
    ...
    // Check seat count from the SELECT query (index 1)
    const currentCount = parseInt(result[1][0].count);
    ...
    // Get registration data from INSERT query (index 2)
    const registration = result[2][0];

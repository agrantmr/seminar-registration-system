...
/**
 * Get the current count of registrations (total seats reserved)
 */
async function getRegistrationCount() {
  try {
    const result = await sql`SELECT COALESCE(SUM(num_seats), 0) as count FROM registrations`;
 ...
 */
async function addRegistration(firstName, email, ipAddress, whereHeard, numSeats = 1) {
  try {
    // Check seat availability first (sum of all num_seats)
    const countResult = await sql`SELECT COALESCE(SUM(num_seats), 0) as count FROM registrations`;
    const currentCount = parseInt(countResult[0].count);
    const totalSeats = parseInt(process.env.TOTAL_SEATS || '40');

    if (currentCount + numSeats > totalSeats) {
      const seatsLeft = totalSeats - currentCount;
      if (seatsLeft === 0) {
        return { success: false, message: 'Event is full' };
      } else {
        return { success: false, message: `Only ${seatsLeft} seat${seatsLeft === 1 ? '' : 's'} remaining. Please reduce the number of seats.` };
      }
    }

    // Try to insert registration
    const result = await sql`
      INSERT INTO registrations (first_name, email, ip_address, where_heard, num_seats)
      VALUES (${firstName}, ${email.toLowerCase()}, ${ipAddress}, ${whereHeard}, ${numSeats})
      RETURNING id, first_name, email, registered_at, num_seats
    `;

    return {
      success: true,
      registration: result[0],
      seatsRemaining: totalSeats - currentCount - numSeats
    };
  } catch (error) {
...
async function getAllRegistrations() {
  try {
    const result = await sql`
      SELECT id, first_name, email, registered_at, where_heard, num_seats,

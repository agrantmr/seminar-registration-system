 ...
 */
async function addFeedback(name, email, feedback, ipAddress) {
  try {
    const result = await sql`
      INSERT INTO feedback (name, email, feedback, ip_address)
      VALUES (${name}, ${email}, ${feedback}, ${ipAddress})
...
async function getAllFeedback() {
  try {
    const result = await sql`
      SELECT id, name, email, feedback, submitted_at
      FROM feedback

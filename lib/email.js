...
async function sendConfirmationEmail(firstName, email) {
  ...
  try {
    ...
    console.log('✉️ Resend API response:', JSON.stringify(result));
 ...
 */
async function sendThankyouEmail(firstName, email) {
  const variables = {
    firstName,
    feedbackUrl: `${process.env.BASE_URL || 'https://proseminars.org'}/feedback.html`
  };

  const html = loadTemplate('thankyou', variables);

  try {
    const result = await resend.emails.send({
      from: process.env.EMAIL_FROM,
      to: email,
      subject: 'Thank You for Attending - A Beginner\'s Guide to AI',
      html
    });
 ...
 */
async function sendAdminNotificationEmail(attendeeName, attendeeEmail, totalRegistrations) {
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2 style="color: #d32f2f;">New Registration!</h2>
      <p>Someone just registered for the seminar:</p>
      <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
        <tr>
          <td style="padding: 8px; border-bottom: 1px solid #ddd; font-weight: bold;">Name:</td>
          <td style="padding: 8px; border-bottom: 1px solid #ddd;">${attendeeName}</td>
        </tr>
        <tr>
          <td style="padding: 8px; border-bottom: 1px solid #ddd; font-weight: bold;">Email:</td>
          <td style="padding: 8px; border-bottom: 1px solid #ddd;">${attendeeEmail}</td>
        </tr>
        <tr>
          <td style="padding: 8px; border-bottom: 1px solid #ddd; font-weight: bold;">Total Registrations:</td>
          <td style="padding: 8px; border-bottom: 1px solid #ddd;">${totalRegistrations}</td>
        </tr>
      </table>
    </div>
  `;
  ...
  try {
    const result = await resend.emails.send({
      from: process.env.EMAIL_FROM,
      to: 'hello@scantochat.ai',
      subject: `New Registration: ${attendeeName}`,
      html,
      reply_to: attendeeEmail
    });

    console.log('✉️ Admin notification sent:', JSON.stringify(result));
    return { success: true, id: result.id };
  } catch (error) {
